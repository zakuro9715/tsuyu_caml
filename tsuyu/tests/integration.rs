...
mod tsuyu_testutil {
    ...
    pub fn run_source(path: impl AsRef<Path>) -> Output {
        ...
        Output::from(tsuyu::run(source).unwrap())
    }
}
...
fn correct_sources() {
    let sources = "sources";
    tsuyu_testutil::walk_files(sources, ".c", |name, path| {
        assert_toml_snapshot!(name, tsuyu_testutil::run_source(path));
    });
}

#[test]
fn compile_error() {
    tsuyu::compile(Source::inline("xx")).expect_err("");
}
